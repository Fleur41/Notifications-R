...
fun NotificationScreen() {
    ...
    Scaffold(
        topBar = {
            TopAppBar(
                title = { Text(text = "NotificationScreen") }
            )
        }
    ) { innerPadding ->
        Column(
            ...
            Button(onClick = {NotificationUtility.createSimpleNotification(context)}) {
                Text(text = "Simple notification")
            }

            Button(onClick = {NotificationUtility.createClickableNotification(context)}) {
                Text(text = "Display a clickable notification")
            }

            Button(onClick = {NotificationUtility.createActionableNotification(context)}) {
                Text(text = "Create actionable notification")
            }
